...
Six runnable lessons. About 60 minutes end to end.
...
- Optional: OTel + Traces in Unity Catalog enabled for lesson 6. The other lessons work without it.
...
The bundle runs all six lessons end to end as a multi-task job on serverless compute.
...
### 5. Schedule scorers in production
...
[`notebooks/05_production_monitoring.py`](notebooks/05_production_monitoring.py)

### 6. Query traces in Unity Catalog
...
[`notebooks/06_otel_uc_integration.py`](notebooks/06_otel_uc_integration.py)
...
├── workshop_scorers.py                        Reusable scorer module (lesson 5 advanced)
├── images/
...
│   ├── 05_production_monitoring.py            Lesson 5
│   ├── 06_otel_uc_integration.py              Lesson 6
│   ├── _resources/
...
| OTel + Traces in UC region availability | Public Preview; not all regions are supported yet. Confirm availability with your account team before relying on lesson 6's SQL examples. |

## What's next

When you finish the six lessons:
...
- Tune the sampling rates in lesson 5 to your judge cost budget
- Configure OTel + Traces in UC in your workspace and wire the SQL queries in lesson 6 to your real catalog
