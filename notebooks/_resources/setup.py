...
# MAGIC
# MAGIC `%run` from any lesson notebook to get a consistent environment:
# MAGIC
# MAGIC - `USER` resolved from the current Databricks identity
# MAGIC - `EXPERIMENT_PATH` set so all lessons write to the same MLflow experiment
# MAGIC - Agent code (`retrieve`, `generate`, `answer_question`) loaded
# MAGIC - `DEMO_QUERIES` available for populating traces
# MAGIC
# MAGIC The agent itself is a small RAG-style example used as the substrate for tracing,
# MAGIC evaluation, and human feedback. The point of the workshop is the platform around
# MAGIC the agent, not the agent itself.
...
# Override here if your workspace has a different endpoint name.
FM_ENDPOINT = "databricks-claude-sonnet-4-6"

# COMMAND ----------

# Mock retrieval corpus. 20 short docs about Databricks GenAI primitives.
# Lets the agent demonstrate retrieve + generate spans with deterministic content.
CORPUS = {
...
def generate(query: str, contexts: list[dict]) -> str:
    """Call the Databricks-hosted Foundation Model with the retrieved context."""
    w = WorkspaceClient()
    if not contexts:
...
def answer_question(
    ...
    """Top-level chain. Sets canonical session/user metadata so the MLflow UI filter chips light up."""
